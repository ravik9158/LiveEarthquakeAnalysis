...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(color(150, 48, 48));
		triangle(50-5, 125+5, 50, 125-5, 50+5, 125+5);
		...
		for(Marker cm : countryMarkers) {
			...
			for(Marker eq : quakeMarkers) {
				if(((EarthquakeMarker) eq).isOnLand()) {
					if((String)eq.getProperty("country") == country) {
						quakes++;
					}
				}
			}
			if(quakes > 0)System.out.println(country +": "+ quakes);
		}
		int oceanquakes = 0;
		for(Marker eq : quakeMarkers) {
			if(!((EarthquakeMarker) eq).isOnLand()) oceanquakes++;
		}
